...
print(seluniongapsrings )
selallbufferdf['mean_per']=seluniongapsrings.values

seluniongapsrings = df_gaps_union.groupby('idrings').percent_ar.std()
print(seluniongapsrings )
selallbufferdf['std_per']=seluniongapsrings.fillna(0).values


print(selallbufferdf )

list_stat = [ 'mean_per', 'std_per',  ]
list_title = ['Mean gap percent (%)', 'Std gap percent (%)']
...
for i in range(grid[0]*grid[1]):
    ...
    selallbufferdf.plot(ax=ax, column=list_stat[i] ,cmap='jet', legend=True)
...
plt.savefig('../output/img_total.png', dpi=500,  facecolor='white', transparent=False)
plt.close()
